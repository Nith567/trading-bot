...
bot.command("start", async (ctx) => {
  ...
  const keyboard = new InlineKeyboard()
    ...
    .row()
    .text("chat with ai", "chat_with_ai")
    .row()
    .text("contribute dao", "contribute_dao")
    .row()
    .text("Buy", "buy")
    .text("Sell", "sell")
    .row()
...
const callbackHandlers = {
  ...
  chat_with_ai:handleInitialChat,
  buy: handleInitialBuy,
  contribute_dao:handleInitialDao,
...
bot.on("message:text", async (ctx) =>
  handleUserState(ctx, async () => {
    ...
    if(userState.chatwithAIRequested) await handleReply(ctx);
    if(userState.DAORequested) await handleDaoReply(ctx);
...
function generateUUID() {
 ...
 const replayhandlerAI = async (destination) => {
  ...
  try {
    const response = await axios.post(
      ...
      {
        input:destination,
        conversation_id: '201a7be3-eb5c-4d08-82cc-09107b9d3051',
      },
      {
        headers: {
    ...
    console.error('Error fetching or processing the response:', error.message);
    throw error;
  }
};

 const handleDaoAPIResponse= async (destination) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/api/DAOcontribute`,
      {
        input:destination,
        conversation_id: '201a7be3-eb5c-4d08-82cc-09107b9d3051',
      },
...
async function handleInitialChat(ctx) {
  updateUserState(ctx.from, { chatwithAIRequested: true });
  await sendReply(
    ctx,
    "chat with our Agent to deploy tokens/contract",
    { reply_markup: { force_reply: true } },
  );
}

async function handleInitialDao (ctx){
  updateUserState(ctx.from, { DAORequested: true });
    await sendReply(
    ctx,
    "contribute to dao baby",
    { reply_markup: { force_reply: true } },
  );
}
async function handleReply(ctx) {
   const userState = userStates[ctx.from.id] || {};
    ...
    try {
      await sendReply(ctx, "finding answer...");
    const res=await replayhandlerAI(destination)
      await sendReply(
        ctx,
       `${res}`
      );
    } catch (error) {
      await ctx.reply("An error occured with ai agent.");
    }

}
async function handleDaoReply(ctx) {
   const userState = userStates[ctx.from.id] || {};

    const destination = ctx.message.text;
    try {
      await sendReply(ctx, "finding answer...");
    const res=await handleDaoAPIResponse(destination)
      await sendReply(
        ctx,
       `${res}`
      );
